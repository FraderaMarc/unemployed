...
@export_file("*.tscn") var paper_scene_path: String = "res://Scenes/Paper.tscn"
...
func _drop_paper() -> void:
	var scene: PackedScene = _get_paper_scene()

	if scene == null:
		push_warning(name + " no puede soltar papel porque no encuentra la escena.")
		return
...
func _get_paper_scene() -> PackedScene:
	if paper_scene != null:
		return paper_scene

	if paper_scene_path != "" and ResourceLoader.exists(paper_scene_path):
		return load(paper_scene_path) as PackedScene
